...
fn compiled_foundation_participation_accepts_subject_and_interaction_facets() {
    ...
    let compiled = compile(&consumer, &[&foundation]);
    assert!(compiled.ir.is_some(), "{}", compiled.render());
    let assembled =
        forgegraph_semantic::assembly::assemble(compiled.ir.as_ref().unwrap(), &[foundation])
            .unwrap();
    ...
    let participant = raw["entities"]
        ...
        .unwrap()
        .keys()
        .find(|k| k.ends_with("/Participant"))
        .unwrap()
        .clone();
