...
fn ts_type(schema: &Value, optional_null: bool) -> String {
    ...
    if optional_null && !base.contains("null") {
        format!("{base} | null")
    } else {
        base
    }
...
pub fn client_ts(c: &Contracts) -> String {
    ...
    let _ = writeln!(
        out,
        "// Generated by forgec build from {} ({}). Do not edit.\n",
        c.package, c.version
    );
    ...
    for r in &c.resources {
        for op in &r.operations {
            ...
            let _ = writeln!(
                out,
                "  {:?}: {{ method: {:?}, path: {:?}, kind: {:?}, resource: {:?} }},",
                op.id, h.method, h.path, op.kind, r.name
            );
        }
    }
    for f in &c.functions {
        if let Some(h) = &f.http {
            let _ = writeln!(
                out,
                "  {:?}: {{ method: {:?}, path: {:?}, kind: \"function\", resource: {:?} }},",
                f.id, h.method, h.path, f.name
            );
        }
    }
    let pkg = &c.package;
    for (action, method, path) in [
        ("propose", "POST", "/v1/changesets"),
        ("get", "GET", "/v1/changesets/{id}"),
        ("preview", "GET", "/v1/changesets/{id}/preview"),
        ("approve", "POST", "/v1/changesets/{id}/approve"),
        ("commit", "POST", "/v1/changesets/{id}/commit"),
    ] {
        let _ = writeln!(
            out,
            "  {:?}: {{ method: {:?}, path: {:?}, kind: {:?}, resource: \"changesets\" }},",
            format!("{pkg}/_/changesets.{action}"),
            method,
            path,
            format!("changeset.{action}")
        );
    }
    for action in ["export", "import", "verify", "fence", "inspect"] {
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"POST\", path: {:?}, kind: {:?}, resource: \"admin\" }},",
            format!("{pkg}/_/admin.{action}"),
            format!("/v1/admin/{action}"),
            format!("admin.{action}")
        );
    }
    let _ = writeln!(
        out,
        "  {:?}: {{ method: \"POST\", path: \"/v1/admin/subjects/locate\", kind: \"admin.subjects.locate\", resource: \"admin\" }},",
        format!("{pkg}/_/admin.subjects.locate")
    );
    for (action, path) in [
        ("inspect", "/v1/imports/inspect"),
        ("stage", "/v1/imports/stage"),
    ] {
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"POST\", path: {:?}, kind: {:?}, resource: \"imports\" }},",
            format!("{pkg}/_/imports.{action}"),
            path,
            format!("import.{action}")
        );
    }
    for v in &c.views {
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"GET\", path: {:?}, kind: \"view.query\", resource: {:?} }},",
            format!("{}.query", v.id),
            v.http.path,
            v.name
        );
    ...
    for p in &c.projections {
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"GET\", path: {:?}, kind: \"projection.get\", resource: {:?} }},",
            format!("{}.get", p.id),
            format!("{}/{{id}}", p.path),
            p.name
        );
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"GET\", path: {:?}, kind: \"projection.status\", resource: {:?} }},",
            format!("{}.status", p.id),
            format!("{}/status", p.path),
            p.name
        );
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"POST\", path: {:?}, kind: \"projection.rebuild\", resource: {:?} }},",
            format!("{}.rebuild", p.id),
            format!("{}/rebuild", p.path),
            p.name
        );
    }
    for k in &c.caches {
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"GET\", path: {:?}, kind: \"cache.read\", resource: {:?} }},",
            format!("{}.read", k.id),
            k.http.path,
            k.name
        );
    ...
    for s in &c.schedules {
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"GET\", path: {:?}, kind: \"schedule.status\", resource: {:?} }},",
            format!("{}.status", s.id),
            s.path,
            s.name
        );
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"POST\", path: {:?}, kind: \"schedule.tick\", resource: {:?} }},",
            format!("{}.tick", s.id),
            format!("{}/tick", s.path),
            s.name
        );
    ...
    for w in &c.workflows {
        let _ = writeln!(
            out,
            "  {:?}: {{ method: {:?}, path: {:?}, kind: \"workflow.start\", resource: {:?} }},",
            format!("{}.start", w.id),
            w.start.method,
            w.start.path,
            w.name
        ...
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"GET\", path: {:?}, kind: \"workflow.get\", resource: {:?} }},",
            format!("{}.get", w.id),
            format!("{}/{{id}}", w.path),
            w.name
        );
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"POST\", path: {:?}, kind: \"workflow.cancel\", resource: {:?} }},",
            format!("{}.cancel", w.id),
        ...
        let _ = writeln!(
            out,
            "  {:?}: {{ method: \"POST\", path: {:?}, kind: \"workflow.signal\", resource: {:?} }},",
            format!("{}.signal", w.id),
    ...
    for r in &c.resources {
        ...
        if let Some(lc) = &r.lifecycle {
            for a in &lc.actions {
                interface(
                    &mut out,
                    &format!("{}{}Input", r.name, upper_first(&a.name)),
                    &a.input,
                );
            }
        }
        ...
        for op in &r.operations {
            ...
            match op.kind.as_str() {
                "create" => {
                    let _ = writeln!(
                        out,
                        "  create(input: {}Create, opts?: CallOptions): Promise<{rec}>;",
                        r.name
                    );
                }
                "get" => {
                    let _ = writeln!(out, "  get(id: string): Promise<{rec}>;");
                }
                "update" => {
                    let _ = writeln!(
                        out,
                        "  update(id: string, expectedVersion: number, patch: {}Patch, opts?: CallOptions): Promise<{rec}>;",
                        r.name
                    );
                }
                "delete" => {
                    let _ = writeln!(
                        out,
                        "  delete(id: string, expectedVersion: number, opts?: CallOptions): Promise<{rec}>;"
                    );
                }
                "restore" => {
                    let _ = writeln!(
                        out,
                        "  restore(id: string, expectedVersion: number, opts?: CallOptions): Promise<{rec}>;"
                    );
                }
                "beginUpload" => {
                    let _ = writeln!(
                        out,
                        "  beginUpload(id: string, expectedVersion: number, input: {{ mediaType: string; byteCount: number }}): Promise<{{ record: {rec}; upload: SignedUrl }}>;"
                    );
                }
                "finalizeUpload" => {
                    let _ = writeln!(
                        out,
                        "  finalizeUpload(id: string, expectedVersion: number): Promise<{rec}>;"
                    );
                }
                "download" => {
                    let _ = writeln!(
                ...
                "effective" => {
                    let _ = writeln!(
                        out,
                        "  effective(params: Record<string, string> & {{ at: string }}): Promise<{rec} | null>;"
                    );
                }
                "move" => {
                    let _ = writeln!(
                        out,
                        "  move(id: string, expectedVersion: number, parent: string | null): Promise<{rec}>;"
                    );
                }
                "children" => {
                    let _ = writeln!(out, "  children(id: string): Promise<Page<{rec}>>;");
                }
                "ancestors" => {
                    let _ = writeln!(out, "  ancestors(id: string): Promise<Page<{rec}>>;");
                }
                "find" | "list" => {
                    let q = r
                        .queries
                    ...
                    let params: Vec<String> = q
                        ...
                        .map(|p| {
                            format!(
                                "{p}: {}",
                                ts_type(r.record.properties.get(p).unwrap_or(&Value::Null), false)
                            )
                        })
                        .collect();
                    ...
                    if op.kind == "find" {
                        let _ = writeln!(
                            out,
                            "  {method}(params: {{ {} }}): Promise<{rec} | null>;",
                            params.join("; ")
                        );
                    } else {
                        let _ = writeln!(
                            out,
                            "  {method}(params: {{ {} }}, page?: PageOptions): Promise<Page<{rec}>>;",
                            params.join("; ")
                        );
                    }
                }
        ...
        if let Some(lc) = &r.lifecycle {
            for a in &lc.actions {
                let exposed = r
                    .operations
                    .iter()
                    .any(|o| o.action.as_ref() == Some(&a.name) && o.http.is_some());
                if exposed {
                    let _ = writeln!(
                        out,
                        "  {}(id: string, expectedVersion: number, input: {}{}Input, opts?: CallOptions): Promise<{}Record>;",
                        a.name,
                        r.name,
                        upper_first(&a.name),
                        r.name
    ...
    for r in &c.resources {
        let _ = writeln!(
            out,
            "  {}: {}Api;",
            camel_from_wire(&plural(&r.wire_name)),
            r.name
        );
    }
    ...
    for v in &c.views {
        ...
        let _ = writeln!(
            out,
            "    {}: {{ query(params: {{ {} }}, page?: PageOptions): Promise<Page<{}Row>> }};",
            lower_first(&v.name),
            params.join("; "),
            v.name
        );
    ...
    for p in &c.projections {
        let _ = writeln!(
            out,
            "    {}: {{ get(id: string): Promise<{}Record>; status(): Promise<ProjectionStatus>; rebuild(): Promise<ProjectionStatus> }};",
            lower_first(&p.name),
            p.name
        );
    }
    ...
    for k in &c.caches {
        ...
        let value = k
            .value_resource
            .as_ref()
            .map(|r| format!("{r}Record | null"))
            .unwrap_or_else(|| "unknown".into());
        let _ = writeln!(
            out,
            "    {}: {{ read(key: {{ {} }}): Promise<CacheRead<{}>> }};",
            lower_first(&k.name),
            keys.join("; "),
            value
        );
    ...
    for s in &c.schedules {
        let _ = writeln!(
            out,
            "    {}: {{ status(): Promise<ScheduleStatus>; tick(now?: string): Promise<{{ results: ScheduleTick[] }}> }};",
            lower_first(&s.name)
        );
    }
    let _ = writeln!(out, "  }};");
    ...
    for w in &c.workflows {
        ...
        let _ = writeln!(
            out,
            "    {}: {{ start(input: Record<string, unknown>, opts?: CallOptions): Promise<WorkflowInstance>; get(id: string): Promise<WorkflowInstance>; cancel(id: string): Promise<WorkflowInstance>; signal(message: {}, payload: Record<string, unknown>, messageId?: string): Promise<{{ delivered: number; held: number }}> }};",
            lower_first(&w.name),
            if msgs.is_empty() {
                "never".to_string()
            } else {
    ...
    let _ = writeln!(
        out,
        "      propose: (input, opts) => t.unwrap(t.call({:?}, input, opts)),",
        format!("{pkg}/_/changesets.propose")
    );
    let _ = writeln!(
        out,
        "      get: (id) => t.unwrap(t.call({:?}, {{ id }})),",
        format!("{pkg}/_/changesets.get")
    );
    let _ = writeln!(
        out,
        "      preview: (id) => t.unwrap(t.call({:?}, {{ id }})),",
        format!("{pkg}/_/changesets.preview")
    );
    let _ = writeln!(
        out,
        "      approve: (id, contentHash) => t.unwrap(t.call({:?}, {{ id, contentHash }})),",
        format!("{pkg}/_/changesets.approve")
    );
    let _ = writeln!(
        out,
        "      commit: (id, opts) => t.unwrap(t.call({:?}, {{ id }}, opts)),",
        format!("{pkg}/_/changesets.commit")
    );
    let _ = writeln!(out, "    }},");
    ...
    for f in c.functions.iter().filter(|f| f.http.is_some()) {
        let _ = writeln!(
            out,
            "      {}: (input, opts) => t.unwrap(t.call({:?}, input, opts)),",
            lower_first(&f.name),
            f.id
        );
    }
    ...
    let _ = writeln!(
        out,
        "      inspect: (input) => t.unwrap(t.call({:?}, input)),",
        format!("{pkg}/_/imports.inspect")
    );
    let _ = writeln!(
        out,
        "      stage: (input) => t.unwrap(t.call({:?}, input)),",
        format!("{pkg}/_/imports.stage")
    );
    let _ = writeln!(out, "    }},");
    let _ = writeln!(out, "    admin: {{");
    let _ = writeln!(
        out,
        "      export: () => t.unwrap(t.call({:?}, {{}})),",
        format!("{pkg}/_/admin.export")
    );
    let _ = writeln!(
        out,
        "      import: (snapshot) => t.unwrap(t.call({:?}, {{ snapshot }})),",
        format!("{pkg}/_/admin.import")
    );
    let _ = writeln!(
        out,
        "      verify: (snapshot) => t.unwrap(t.call({:?}, {{ snapshot }})),",
        format!("{pkg}/_/admin.verify")
    );
    let _ = writeln!(
        out,
        "      fence: (on) => t.unwrap(t.call({:?}, {{ on }})),",
        format!("{pkg}/_/admin.fence")
    );
    let _ = writeln!(
        out,
        "      locateSubject: (input) => t.unwrap(t.call({:?}, input)),",
        format!("{pkg}/_/admin.subjects.locate")
    );
    let _ = writeln!(
        out,
        "      inspect: (input) => t.unwrap(t.call({:?}, input)),",
        format!("{pkg}/_/admin.inspect")
    );
    let _ = writeln!(out, "    }},");
    let _ = writeln!(out, "    views: {{");
    for v in &c.views {
        let _ = writeln!(
            out,
            "      {}: {{ query: (params, page) => t.unwrap(t.call({:?}, {{ params, ...(page ?? {{}}) }})) }},",
            lower_first(&v.name),
            format!("{}.query", v.id)
        );
    }
    ...
    for p in &c.projections {
        let _ = writeln!(
            out,
            "      {}: {{ get: (id) => t.unwrap(t.call({:?}, {{ id }})), status: () => t.unwrap(t.call({:?}, {{}})), rebuild: () => t.unwrap(t.call({:?}, {{}})) }},",
            lower_first(&p.name),
            format!("{}.get", p.id),
            format!("{}.status", p.id),
            format!("{}.rebuild", p.id)
    ...
    for s in &c.schedules {
        let _ = writeln!(
            out,
            "      {}: {{ status: () => t.unwrap(t.call({:?}, {{}})), tick: (now) => t.unwrap(t.call({:?}, now ? {{ now }} : {{}})) }},",
            lower_first(&s.name),
            format!("{}.status", s.id),
            format!("{}.tick", s.id)
        );
    ...
    for w in &c.workflows {
        let _ = writeln!(
            out,
            "      {}: {{ start: (input, opts) => t.unwrap(t.call({:?}, input, opts)), get: (id) => t.unwrap(t.call({:?}, {{ id }})), cancel: (id) => t.unwrap(t.call({:?}, {{ id }})), signal: (message, payload, messageId) => t.unwrap(t.call({:?}, {{ message, payload, ...(messageId ? {{ messageId }} : {{}}) }})) }},",
            lower_first(&w.name),
            format!("{}.start", w.id),
            format!("{}.get", w.id),
            format!("{}.cancel", w.id),
    ...
    for r in &c.resources {
        ...
        for op in &r.operations {
            ...
            match op.kind.as_str() {
                "create" => {
                    let _ = writeln!(
                        out,
                        "      create: (input, opts) => t.unwrap(t.call({id:?}, input, opts)),"
                    );
                }
                "get" => {
                    let _ = writeln!(
                        out,
                        "      get: (id) => t.unwrap(t.call({id:?}, {{ id }})),"
                    );
                }
                "update" => {
                    let _ = writeln!(
                        out,
                        "      update: (id, expectedVersion, patch, opts) => t.unwrap(t.call({id:?}, {{ id, expectedVersion, patch }}, opts)),"
                    );
                }
                "delete" => {
                    let _ = writeln!(
                        out,
                        "      delete: (id, expectedVersion, opts) => t.unwrap(t.call({id:?}, {{ id, expectedVersion }}, opts)),"
                    );
                }
                "restore" => {
                    let _ = writeln!(
                        out,
                        "      restore: (id, expectedVersion, opts) => t.unwrap(t.call({id:?}, {{ id, expectedVersion }}, opts)),"
                    );
                }
                "beginUpload" => {
                    let _ = writeln!(
                        out,
                        "      beginUpload: (id, expectedVersion, input) => t.unwrap(t.call({id:?}, {{ id, expectedVersion, ...input }})),"
                    );
                }
                "finalizeUpload" => {
                    let _ = writeln!(
                        out,
                        "      finalizeUpload: (id, expectedVersion) => t.unwrap(t.call({id:?}, {{ id, expectedVersion }})),"
                    );
                }
                "download" => {
                    let _ = writeln!(
                        out,
                        "      download: (id) => t.unwrap(t.call({id:?}, {{ id }})),"
                    );
                }
                "effective" => {
                    let _ = writeln!(
                        out,
                        "      effective: (params) => t.unwrapNullable(t.call({id:?}, {{ params }})),"
                    );
                }
                "move" => {
                    let _ = writeln!(
                        out,
                        "      move: (id, expectedVersion, parent) => t.unwrap(t.call({id:?}, {{ id, expectedVersion, parent }})),"
                    );
                }
                "children" => {
                    let _ = writeln!(
                        out,
                        "      children: (id) => t.unwrap(t.call({id:?}, {{ id }})),"
                    );
                }
                "ancestors" => {
                    let _ = writeln!(
                        out,
                        "      ancestors: (id) => t.unwrap(t.call({id:?}, {{ id }})),"
                    );
                }
                "find" => {
                    let _ = writeln!(
                        out,
                        "      find{}: (params) => t.unwrapNullable(t.call({id:?}, {{ params }})),",
                        upper_first(op.query.as_ref().unwrap())
                    );
                }
                "list" => {
                    let _ = writeln!(
                        out,
                        "      list{}: (params, page) => t.unwrap(t.call({id:?}, {{ params, ...page }})),",
                        upper_first(op.query.as_ref().unwrap())
                    );
                }
                _ => {}
        ...
        if let Some(lc) = &r.lifecycle {
            for a in &lc.actions {
                ...
                if r.operations
                    .iter()
                    .any(|o| o.id == opid && o.http.is_some())
                {
                    let _ = writeln!(
                        out,
                        "      {}: (id, expectedVersion, input, opts) => t.unwrap(t.call({opid:?}, {{ id, expectedVersion, input }}, opts)),",
                        a.name
                    );
                }
            }
