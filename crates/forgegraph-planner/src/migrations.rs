...
use forgegraph_semantic::diff::Report;
use serde::Serialize;
...
pub fn plan_migration(report: &Report, old: &Value, new: &Value) -> MigrationPlan {
        ...
        match f.code {
            "column-added" => {
                ...
                let ddl = column(new, table, col).map(|c| {
                    let ty = s(&c["sqlType"]);
                    let null = if c["nullable"] == Value::Bool(false) {
                        " NOT NULL DEFAULT ''".to_string()
                    } else {
                        String::new()
                    };
                    Ddl {
                        sqlite: format!("ALTER TABLE {table} ADD COLUMN {col} {ty}{null};"),
                        postgres: format!(
                ...
                push(
                    ...
                    "storage.expand",
                    f.subject.clone(),
                    "additive column before the new readers deploy".into(),
                    false,
                    false,
                    ddl,
                );
            ...
            "table-added" => {
                push(
                    ...
                    "storage.expand",
                    f.subject.clone(),
                    "create the table from the new bundle's DDL before the new readers deploy"
                        .into(),
                    false,
                    false,
                    None,
            ...
            "column-type-changed" => {
                push(
                    ...
                    f.subject.clone(),
                    format!(
                        "{}; run as an idempotent, resumable rewrite; the affected volume is measured by the rollout, not assumed",
                        f.detail
                    ),
                    true,
                    false,
            ...
            "column-removed" | "table-removed" => {
                let (table, col) = f.subject.split_once('.').unwrap_or((&f.subject, ""));
                let ddl = if f.code == "column-removed" {
                    Some(Ddl {
                        sqlite: format!("ALTER TABLE {table} DROP COLUMN {col};"),
                        postgres: format!("ALTER TABLE {table} DROP COLUMN {col};"),
                    })
                } else {
                    Some(Ddl {
                        sqlite: format!("DROP TABLE {table};"),
                        postgres: format!("DROP TABLE {table};"),
                    })
                };
            ...
            "subject-binding-added" => {
                push(
                    ...
                    "subject.index",
                    f.subject.clone(),
                    "build the subject index for existing records (resumable)".into(),
                    false,
                    false,
                    None,
                );
            ...
            "subject-binding-removed" | "subject-binding-changed" => {
                push(
                    ...
                    "subject.review",
                    f.subject.clone(),
                    "subject rights (access/erasure) depend on this binding: review before rollout"
                        .into(),
                    true,
                    false,
                    None,
            ...
            "personal-field-added" => {
                ...
                let scoped = new["ir"]["modules"]
                    .as_array()
                    .into_iter()
                    .flatten()
                    .flat_map(|m| m["resources"].as_array().into_iter().flatten())
                    .any(|r| {
                        r["id"] == rid && r["decorators"]["purposeScoped"] == Value::Bool(true)
                    });
                let mapped = new["capabilities"]["surfaces"]
                    .as_array()
                    .into_iter()
                    .flatten()
                    .any(|sf| {
                        sf["resource"] == rid
                            && sf["allowAtoms"]
                                ...
                                .into_iter()
                                .flatten()
                                .any(|a| a["verb"] == "read" && a["name"] == field)
                    });
                if !scoped {
            ...
            "dependency-removed" => {
                push(
                    ...
                    "grant.retire",
                    f.subject.clone(),
                    "retire the grant after releases still using it drain".into(),
                    true,
                    false,
                    None,
                );
            ...
            "graph-changed-without-version" => {
                push(
                    ...
                    "workflow.version",
                    f.subject.clone(),
                    "bump `version` before rollout: in-flight instances are pinned".into(),
                    true,
                    true,
                    None,
                );
            ...
            "policy-changed-unknown" => {
                push(
                    ...
                    "policy.review",
                    f.subject.clone(),
                    "the policy change has no supported proof; treat as changed and review".into(),
                    true,
                    false,
                    None,
                );
            ...
            "policy-narrowed" => {
                push(
                    ...
                    "decision.epoch",
                    f.subject.clone(),
                    "bump the decision epoch so cached allows are re-decided".into(),
                    false,
                    false,
                    None,
                );
    ...
    let (ot, nt) = (
        s(&old["dataSemantics"]["taxonomy"]),
        s(&new["dataSemantics"]["taxonomy"]),
    );
    ...
    let projections = |v: &Value| -> Vec<(String, String)> {
        v["ir"]["modules"]
            ...
            .into_iter()
            .flatten()
            .flat_map(|m| m["projections"].as_array().into_iter().flatten())
            .map(|p| (s(&p["id"]), serde_json::to_string(p).unwrap_or_default()))
            .collect()
    };
    let (op, np) = (projections(old), projections(new));
    for (id, def) in &np {
        match op.iter().find(|(i, _)| i == id) {
            Some((_, odef)) if odef != def => push(
                "backfill",
                "projection.rebuild",
                id.clone(),
                "definition changed: rebuild from the source stream (resumable, idempotent)".into(),
                false,
                false,
                None,
            ),
            None => push(
                ...
                "projection.rebuild",
                id.clone(),
                "new projection: build from the source stream".into(),
                false,
                false,
                None,
            ),
    ...
    push(
        "traffic",
        "traffic.shift",
        report.new_build.clone(),
        "shift traffic by stages with the SLO gate between them".into(),
        false,
        false,
        None,
    );
    ...
    MigrationPlan {
        ...
        steps,
        unknown: vec![
            "affected row counts (measured during backfill, never estimated here)",
            "live traffic and error rates (measured by the SLO gate)",
            "which releases still use retiring grants (reported by the grant registry)",
        ],
    }
}
