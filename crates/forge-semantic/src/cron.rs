...
pub fn validate(cron: &str) -> Result<(), String> {
    ...
    let ranges = [(0u32, 59u32, "minute"), (0, 23, "hour"), (1, 31, "day-of-month"), (1, 12, "month"), (0, 7, "day-of-week")];
    for (text, (min, max, name)) in fields.iter().zip(ranges) {
        ...
        for part in text.split(',') {
            let (base, step) = match part.split_once('/') {
                Some((b, s)) => (b, s.parse::<u32>().map_err(|_| format!("bad step in {name} `{part}`"))?),
                None => (part, 1),
            };
            let (lo, hi) = if base == "*" {
                (min, max)
            } else if let Some((a, b)) = base.split_once('-') {
                (a.parse::<u32>().map_err(|_| format!("bad {name} `{part}`"))?, b.parse::<u32>().map_err(|_| format!("bad {name} `{part}`"))?)
            } else {
                let v = base.parse::<u32>().map_err(|_| format!("bad {name} `{part}`"))?;
                (v, v)
            };
...
pub fn known_timezone(tz: &str) -> bool {
    tz == "UTC" || (tz.contains('/') && tz.chars().all(|c| c.is_ascii_alphanumeric() || matches!(c, '/' | '_' | '-' | '+')))
}
