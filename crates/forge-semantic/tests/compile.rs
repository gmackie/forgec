use forge_semantic::{Package, compile, load_package};
use std::path::Path;
...
fn workflow_lowers_to_a_step_graph_with_stable_ids_and_checks() {
    let ir = forge_semantic::compile(
        &forge_semantic::load_package(&examples().join("acme")).unwrap(),
        &[&forge_semantic::compile(
            &forge_semantic::load_package(&examples().join("payments")).unwrap(),
            &[],
        )
        .ir
        .unwrap()],
...
fn edition_2027_governance_declarations_lower_and_are_gated() {
    ...
    assert_eq!(
        ...
        Some(forge_semantic::ir::SubjectBinding::Kind("person"))
    );
    assert_eq!(
        contact
            .capabilities
...
fn ir_loading_fails_closed_on_unknown_critical_features() {
    ...
    assert!(
        forge_semantic::ir::DomainIR::load(&v)
            .unwrap_err()
...
fn classification_subjects_and_lineage_lower_conservatively() {
    ...
    assert_eq!(
        ...
        Some(forge_semantic::ir::SubjectBinding::From("student"))
    );
