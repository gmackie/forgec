...
fn collect(node: &SyntaxNode, out: &mut Vec<Tok>) {
    let has_header_decorators = header_kind(node.kind()) && node.children().any(|c| c.kind() == K::DECORATOR);
        ...
        match el {
            ...
            NodeOrToken::Node(n) => {
                ...
                if n.kind() == K::ORDER_LIST {
                    ...
                    if let Some(i) = (0..start).rev().find(|&i| out[i].kind == K::IDENT && out[i].text == "order" && out[i].parent == K::LIST_DECL) {
...
fn tight_before(t: &Tok, prev: &Tok) -> bool {
    match t.kind {
        ...
        _ => matches!(prev.kind, K::L_PAREN | K::L_BRACKET | K::AT | K::DOT | K::DOT_DOT | K::BANG)
        ...
        match t.kind {
            ...
            K::NEWLINE => {
                if line.is_empty() {
                    ...
                    flush(&mut out, &mut line, line_indent, &mut pending_blank, &mut lines_emitted);
                    if continuation_indent.is_some() && depth_stack.last().copied() != continuation_indent {
