...
/// PostgreSQL dialect (FORGE-033). Same relational plan as SQLite/D1 with: 64-bit integers for
/// `integer` and minor-unit money, `COLLATE "C"` on every text column so keyset ordering matches the
/// byte order D1 and DynamoDB use, a boolean guarded-batch assertion row, and the outbox trace
/// column, since a new target starts from this baseline rather than the D1 migration stream.
pub fn render_postgres(s: &SqlSchema) -> String {
    fn pg_type(c: &Column) -> String {
        match c.sql_type.as_str() {
            "INTEGER" => "BIGINT".into(),
            "TEXT" => "TEXT COLLATE \"C\"".into(),
            other => other.into(),
        }
    }
