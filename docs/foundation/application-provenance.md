...
| Bob `f388e25f4a1e796ac9a9f07e8aa7478484d32b02` | `foundationFulfillmentService.ts`, exported from planningWriteService | Reconciles a persisted completed taskRun into FulfillmentEnd and RunCompletion. Requires an independently recorded start; never treats row creation as execution start. Trace controls the application DB read and executes the actual service and Foundation writes; it does not run a Bob agent. |
...
- Bob: current main at f388e25f includes the strengthened reconciliation service from independently merged PR #210. Twelve real PGlite native tests and API typechecking pass; the old pinned branch is historical.
- LatchFlow: [clean current-main draft #169](https://git.forgegraf.com/gmackie/latchflow/pulls/169), four owned files at a36ec961. All16 native tests and API typechecking pass; the older divergent branch is historical.
