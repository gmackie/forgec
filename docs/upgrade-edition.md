...
`forgec upgrade-edition <package>` prints proposals and writes nothing:
...
by hand. `forgec compat` between the two builds shows only `governance / surface-added` findings (new
authority to review) and nothing in the storage or dependencies streams
(`crates/forgegraph-cli/tests/cli.rs::edition_upgrade_is_explicit_and_non_destructive`).
