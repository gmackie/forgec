...
describe("deployment inventory", () => {
  ...
  let base: string;
  beforeAll(async () => {
    host = createNodeHost({ auth: devHeaderAuth(), bundle, store: new MemoryStorage(), functions, externals, cursorSecret: "inv-test", sweepIntervalMs: 0, telemetryFormat: "silent" });
