...
| `grants` / `approval-bot` / `grant-publication` | Callee-owned dependency grants (M16): `requestFrom` (IR uses → signed request), `ApprovalBot` (idempotent callee PR under a restricted path, no execution), `publishGrant` (protected-branch merge, per-group independent approvals on the merged head, signed grant), `GrantRegistry` (admit → activate → exact non-transitive verify; graphs; retire/drain; emergencyRevoke; reviewUnused), `guardExternals`. See `docs/grant-lifecycle.md`. |
