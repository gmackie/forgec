...
## Durable composed acceptance (#71)

`prepareAcceptance` creates an immutable AgreementAcceptance with the same exact pins, approved Decision, participants and server-clock validity rules as direct acceptance. It additionally pins copies of the Offer interval, each constrained equal to the referenced Offer. It does not issue or create an Agreement. A composer atomically publishes its winning terminal and an AgreementAcceptanceCommit through Engine.atomic; the commit repeats exact intent fields and independently checks all acceptance windows against its own server timestamp. This avoids trusting a caller's historical time or an expired preparation. QuoteEnd separately checks its actual publication time against quote expiry, offer validity and Agreement start.

`acceptCommitted(command, commitmentId, context)` reads both durable records, validates current authorization and the authoritative Decision/signers, requires an exact original command, and creates an Agreement referencing the commit. Agreement schema pins every field to that commit; only the original createdAt-window checks are replaced by the commit's already-validated windows. Attribution remains the original accepter even when another authorized actor resumes. No public recovery method accepts an uncommitted intent. Recovery rereads signer terminal facts and rejects any end effective at or before the commit. Quote composition additionally guards each null signer-end observation inside the atomic commit; other composers must provide equivalent serializable checks. Raw commit creation requires ordinary write authorization and represents a new lower-level acceptance constrained by current windows, independent of any higher quote workflow.

The optional Agreement.acceptance reference preserves ordinary direct acceptance behavior. Issuance after expiry never changes validity or reactivates rights; history records actual Agreement/issuance creation times. New acceptance tables and nullable references require storage migration. No legacy historical acceptance timestamps are inferred or backfilled.
