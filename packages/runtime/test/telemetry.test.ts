...
describe("operation events", () => {
  ...
  it("never labels events with unbounded values: no tenant, ids or input in the dimensions", async () => {
    ...
    expect(Object.keys(dims).sort()).toEqual(["forge.kind", "forge.operation", "forge.outcome", "forge.resource", "forge.target"]);
  ...
  it("formats CloudWatch EMF with the plan's histogram and counters (no high-cardinality dimensions)", () => {
    const ev: OperationEvent = { ts: "2026-09-20T00:00:00.000Z", seq: 1, operation: "@acme/commerce/_/Customer.create", kind: "create", resource: "@acme/commerce/_/Customer", outcome: "good", status: 201, durationMs: 12.5, logical: true, attempt: 1, requestId: "r", target: "aws-dynamodb" };
