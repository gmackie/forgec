...
function setup(adapter: string) {
  ...
  const executor: SqlExecutor = {
    ...
    batch: async statements => { db.exec("BEGIN"); try { const result = statements.map(execute); db.exec("COMMIT"); return result; } catch (e) { db.exec("ROLLBACK"); throw e; } },
  };
  ...
  return { engine, api, call, query, close: () => db.close() };
}
for (const adapter of ["memory", "sqlite"]) describe(adapter, () => {
