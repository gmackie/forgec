...
export class Quotations{
 ...
 private price(quote:Wire,ctx:CallContext):Effect.Effect<{lines:Wire[];totals:Record<string,string>},ForgeError>{const self=this;return Effect.gen(function*(){
  ...
  if(quote.evaluation!=null){const finish=yield* self.engine.call(ep+"EvaluationFinish.get",{id:quote.evaluation},ctx),run=yield* self.engine.call(ep+"EvaluationRun.get",{id:finish.run},ctx);if(finish.outcome!=="Completed"||finish.start==null)return yield* bad("Pricing evaluation is incomplete");const start=yield* self.engine.call(ep+"EvaluationStart.get",{id:finish.start},ctx);if(start.run!==run.id||String(finish.finishedAt)<String(start.startedAt))return yield* bad("Invalid pricing evaluation chronology");yield* self.engine.call(ep+"EvaluationExecutor.get",{id:run.executor},ctx);yield* self.engine.call(sp+"SpecificationPin.get",{id:run.definition},ctx);if(finish.support!=null){const seal=yield* self.engine.call("@forgegraph/foundation/evidence/_/EvidenceSeal.get",{id:finish.support},ctx);yield* new Evidence(self.engine).sealedItems(String(seal.bundle),ctx);}}
